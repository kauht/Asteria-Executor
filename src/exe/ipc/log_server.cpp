...
bool LogServer::Start() {
  ...
  hPipe =
      CreateNamedPipeA(LOG_PIPE_NAME, PIPE_ACCESS_INBOUND,
                       PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1,
                       MAX_BUFFER_SIZE, MAX_BUFFER_SIZE, 0, NULL);
...
void LogServer::Run() {
  ...
  while (bRunning) {
    ...
    if (hPipe == INVALID_HANDLE_VALUE) {
      hPipe = CreateNamedPipeA(LOG_PIPE_NAME, PIPE_ACCESS_INBOUND,
                               PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE |
                                   PIPE_WAIT,
                               1, MAX_BUFFER_SIZE, MAX_BUFFER_SIZE, 0, NULL);
      if (hPipe == INVALID_HANDLE_VALUE) {
        Sleep(200);
        continue;
      }
