...
bool Injector::InjectDll(DWORD pid, const std::string &dllPath) {
  ...
  void *pRemoteBuf = VirtualAllocEx(hProcess, NULL, dllPath.length() + 1,
  ...
  if (!WriteProcessMemory(hProcess, pRemoteBuf, dllPath.c_str(),
...
BOOL Injector::UnhookMethod(HANDLE hProcess, const char *methodName,
