...
template <typename T> inline void SafeWrite(uintptr_t addr, T value) {
  if (!addr)
    return;
  __try {
    *(T *)addr = value;
...
void Init() {
  if (regenAddr == 0) {
    regenAddr = (uintptr_t)ScanPattern(
        "client.dll", "48 83 EC ? E8 ? ? ? ? 48 85 C0 0F 84 ? ? ? ? 48 8B 10");
    if (regenAddr) {
      uint16_t combinedOffset = static_cast<uint16_t>(
          m_AttributeManager + m_Item + m_AttributeList + m_Attributes);
      DWORD oldProtect;
      if (VirtualProtect(reinterpret_cast<void *>(regenAddr + 0x52), 2,
                         PAGE_EXECUTE_READWRITE, &oldProtect)) {
        *reinterpret_cast<uint16_t *>(regenAddr + 0x52) = combinedOffset;
        VirtualProtect(reinterpret_cast<void *>(regenAddr + 0x52), 2,
  ...
  if (updateSubclassAddr == 0) {
    updateSubclassAddr = (uintptr_t)ScanPattern(
        "client.dll", "40 53 48 83 EC 30 48 8B D9 E8 ? ? ? ? 48 8B 03");
  }
}
...
void CreateAttributes(uintptr_t item, int paintKit, int seed, float wear) {
  uintptr_t attrListAddr =
      item + m_AttributeManager + m_Item + m_AttributeList + m_Attributes;
...
void ApplyAndRegen(uintptr_t weapon, const SkinConfig &skin,
                   uint16_t defIndex) {
...
void TickInner() {
  ...
  // Getting pawn explicitly handled since standard dwLocalPlayerPawn isn't
  // saved directly in GameContext.
  uintptr_t dwLocalPlayerPawn =
      cs2_dumper::offsets::client_dll::dwLocalPlayerPawn;
  uintptr_t localPawn =
      SafeRead<uintptr_t>(GameContext::Get().client_base + dwLocalPlayerPawn);
  if (!localPawn)
    return;
  ...
  if (activeWeapon) {
    ...
    bool isWeapon =
        (defIndex > 0 && defIndex < 70) || (defIndex >= 500 && defIndex < 600);
    if (isWeapon && defIndex != 31) {
      if (defIndex >= 500 && knifeSkin.enabled && knifeSkin.paintKit > 0) {
        // Knife flow
        bool needsApply = force || (activeWeapon != lastAppliedWeapon) ||
                          (knifeSkin.paintKit != lastAppliedKit) ||
                          (knifeSkin.defIndex != lastAppliedKnifeDef);
        if (needsApply) {
          SkinConfig tempSkin;
          ...
          if (knifeSkin.defIndex != 0) {
            SafeWrite<uint16_t>(item + m_iItemDefinitionIndex,
                                knifeSkin.defIndex);
        ...
        if (defIndex == 42 || defIndex == 59) {
          for (auto &[key, cfg] : weaponSkins)
            if (key >= 500 && key < 600 && cfg.enabled) {
              lookupIndex = key;
              break;
            }
        }
