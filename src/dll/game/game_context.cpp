...
bool GameContext::Init() {
  ...
  create_move_addr =
      ScanPattern("client.dll", "48 89 5C 24 ?? 55 57 41 56 48 8D 6C 24 ?? 48 "
                                "81 EC ?? ?? ?? ?? 8B 01 48 8B F9");

  // Resolve CCSGOInput Helpers
  uintptr_t input_addr = (uintptr_t)ScanPattern(
      "client.dll", "48 8B 0D ? ? ? ? E8 ? ? ? ? 48 8B CF");
  if (input_addr) {
  ...
  GetCUserCmdTick = (FnGetCUserCmdTick)ScanPattern(
      "client.dll",
      "48 83 EC ? 4C 8B 0D ? ? ? ? 48 8B F9 8B 0A 33 DB 4D 8B F0");
  GetCUserCmdArray = (FnGetCUserCmdArray)ScanPattern(
      "client.dll", "48 89 4C 24 ? 41 56 41 57 48 83 EC ? 4C 63 FA 4C 8B F1");
  GetCUserCmdBySequenceNumber = (FnGetCUserCmdBySequenceNumber)ScanPattern(
      "client.dll", "40 53 48 83 EC ? 8B 0D ? ? ? ? 48 8D 05 ? ? ? ? 48 89 05");
  ...
  if (g_Lua) {
    char buf[512];
    sprintf_s(buf,
              "[Init] pCCSGOInput: %p | GetCUserCmdTick: %p | "
              "GetCUserCmdArray: %p | GetCUserCmdBySequenceNumber: %p",
              pCCSGOInput, GetCUserCmdTick, GetCUserCmdArray,
...
void *GameContext::GetCurrentUserCmd(void *controller) {
  ...
  if (!pCCSGOInput || !GetCUserCmdTick || !GetCUserCmdArray ||
      !GetCUserCmdBySequenceNumber || !controller) {
    if (g_Lua)
      g_Lua->GetLogClient().SendLog(
          std::string("[Cmd] Missing pointers or controller"));
    return nullptr;
  }
  ...
  if (!identity) {
    if (g_Lua)
      g_Lua->GetLogClient().SendLog(
          std::string("[Cmd] Step 0: Failed to read CEntityIdentity"));
    return nullptr;
  }
  ...
  if (g_Lua)
    g_Lua->GetLogClient().SendLog(std::string("[Cmd] Step 1: GetCUserCmdTick"));
  ...
  if (g_Lua)
    g_Lua->GetLogClient().SendLog(std::string("[Cmd] Step 2: อ่าน first_array"));
  void **first_array = *(void ***)pCCSGOInput;
  if (!first_array) {
    if (g_Lua)
      g_Lua->GetLogClient().SendLog(std::string("[Cmd] first_array is NULL"));
    return nullptr;
  }

  if (g_Lua)
    g_Lua->GetLogClient().SendLog(
        std::string("[Cmd] Step 3: GetCUserCmdArray"));
  void *arr = GetCUserCmdArray(first_array, tick - 1);
  if (!arr) {
    if (g_Lua)
      g_Lua->GetLogClient().SendLog(
          std::string("[Cmd] GetCUserCmdArray returned NULL"));
    return nullptr;
  }

  if (g_Lua)
    g_Lua->GetLogClient().SendLog(
        std::string("[Cmd] Step 4: Resolve Sequence"));
  uint32_t seq = *(uint32_t *)((uintptr_t)arr + 0x59A8);

  if (g_Lua)
    g_Lua->GetLogClient().SendLog(
        std::string("[Cmd] Step 5: GetCUserCmdBySequenceNumber"));
