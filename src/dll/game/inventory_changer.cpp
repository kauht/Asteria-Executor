...
static void* safe_read_ptr(uintptr_t addr) {
    __try { return *reinterpret_cast<void**>(addr); }
    __except (EXCEPTION_EXECUTE_HANDLER) { return nullptr; }
}

static int safe_read_int(uintptr_t addr) {
    __try { return *reinterpret_cast<int*>(addr); }
...
static void safe_write_u64(uintptr_t addr, uint64_t val) {
    __try { *reinterpret_cast<uint64_t*>(addr) = val; }
    __except (EXCEPTION_EXECUTE_HANDLER) { }
}

static void safe_write_u16(uintptr_t addr, uint16_t val) {
    __try { *reinterpret_cast<uint16_t*>(addr) = val; }
    __except (EXCEPTION_EXECUTE_HANDLER) { }
}
...
static void* call_create_type_cache(FnCreateBaseTypeCache fn, void* socache, int classID) {
    __try { return fn(socache, classID); }
    __except (EXCEPTION_EXECUTE_HANDLER) { return nullptr; }
}
...
static bool call_socreated_ptr(void* pInv, SOID_t* pOwner, void* pItem, int evt) {
    __try {
        void** vt = *reinterpret_cast<void***>(pInv);
...
bool InventoryChanger::AddItemToInventory(CEconItem* pItem) {
    ...
    safe_write_u64((uintptr_t)pItem + OFF_CEconItem_ID,        14000000000ULL + (uint64_t)nCount + 1);
    safe_write_u32((uintptr_t)pItem + OFF_CEconItem_AccountID, accountID);
    safe_write_u32((uintptr_t)pItem + OFF_CEconItem_Inventory, 3000 + (uint32_t)nCount + 1);
    ...
    if (!pSOCache) { lastErrorCode = 6; return false; }
    ...
    if (!pTypeCache) { lastErrorCode = 7; return false; }

    // 6. AddObject (VFunc 1)
    bool bAdded = call_addobject(pTypeCache, pItem);
    if (!bAdded) { lastErrorCode = 8; return false; }
    ...
    SOID_t ownerVal = {};
    __try { ownerVal = *pOwner; } __except (EXCEPTION_EXECUTE_HANDLER) {}
    
    typedef void (__fastcall* FnSOCreated)(void*, SOID_t*, void*);
    FnSOCreated oSOCreated = (FnSOCreated)safe_read_ptr((uintptr_t)safe_read_ptr((uintptr_t)pInv));
    if (oSOCreated) {
        __try {
            oSOCreated(pInv, &ownerVal, pItem);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
            lastErrorCode = 13; return false;
        }
    } else {
        lastErrorCode = 13; return false;
    }
...
int InventoryChanger::GetInventoryCount() {
    if (!getInventoryManagerAddr) return -1;
    void* pMgr = call_get_mgr(getInventoryManagerAddr);
    if (!pMgr) return -2;
    void* pInv = call_vfunc_ptr(pMgr, 70);
    if (!pInv) return -3;
...
int InventoryChanger::GetEconItemRawWord(int index, int wordIndex) {
    ...
    void* pSOCache = safe_read_ptr((uintptr_t)pInv + OFF_INV_SO_CACHE);
    if (!pSOCache) return -4;

    void* pTypeCache = nullptr;
    if (oCreateBaseTypeCache)
        pTypeCache = call_create_type_cache(oCreateBaseTypeCache, pSOCache, 1);
    if (!pTypeCache) return -5;

    int size = safe_read_int((uintptr_t)pTypeCache + 8); 
    uintptr_t elements = (uintptr_t)safe_read_ptr((uintptr_t)pTypeCache + 16);
    if (!elements) return -7; // Adjusted error code

    if (index < 0 || index >= size) return -6;

    uintptr_t pItem = (uintptr_t)safe_read_ptr(elements + index * 8); // CUtlVector stores pointers (8 bytes each)
    if (!pItem) return -7;

    if (wordIndex < 0 || wordIndex > 17) return -8; // CEconItem is 72 bytes max (18 words * 4 bytes/word)
...
int InventoryChanger::GetEconItemCacheCount() {
    ...
    // Debugging print
    char buf[256];
    sprintf_s(buf, "[MyCheat] pInv: %p | SOCache: %p | TypeCache: %p | Size: %d | Elements: %p", 
        pInv, pSOCache, pTypeCache, size, (void*)elements);
    
    extern LuaManager* g_Lua;
    if (g_Lua) g_Lua->GetLogClient().SendLog(buf);

    return size;
}

int InventoryChanger::GetLastErrorCode() {
...
void InventoryChanger::ScanInventoryManager() {
    ...
    if (!getInventoryManagerAddr) { scanResults.push_back("Manager addr not found"); return; }
    void* pMgr = call_get_mgr(getInventoryManagerAddr);
    for (int r = 0; r < 20 && !pMgr; ++r) {
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
        pMgr = call_get_mgr(getInventoryManagerAddr);
    }
    if (!pMgr) { scanResults.push_back("Manager Null After Retries"); return; }
    ...
    if (pInv) {
        ...
        snprintf(buf, sizeof(buf), "CreateBaseTypeCache fn: %p", (void*)oCreateBaseTypeCache);
        scanResults.push_back(buf);
    }
}

int InventoryChanger::GetScanResultCount() {
    return (int)scanResults.size();
}

std::string InventoryChanger::GetScanResult(int index) {
    if (index < 0 || index >= (int)scanResults.size()) return "";
