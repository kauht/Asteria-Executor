...
typedef bool(__fastcall* CreateMoveFn)(void*, void*, void*, void*, void*, void*);

#include "../game/skin_changer.h"

bool __fastcall Detour_CreateMove(void* rcx, void* rdx, void* r8, void* r9, void* arg5, void* arg6) {
    SkinChanger::Tick();
    ...
    if (ref != -1 && g_Lua) {
        ...
        if (L) {
            ...
            if (lua_pcall(L, 4, 0, 0) != 0) {
                ...
                if (err) {
                    g_Lua->GetLogClient().SendLog(std::string("CreateMove Callback Error: ") + err);
                }
                lua_pop(L, 1);
            }
        }
...
bool HookManager::HookCreateMove(void* pTarget) {
    ...
    int status = MH_CreateHook(pTarget, &Detour_CreateMove, &m_OriginalCreateMove);
    if (status != MH_OK) {
        if (g_Lua) {
            g_Lua->GetLogClient().SendLog(std::string("MH_CreateHook Failed: ") + std::to_string(status));
        }
        return false;
    }
